 ...
 *
 * core/button に「LINE 緑」(is-style-button-line) が選択されたとき、Button Icon 属性
 * (customIconSvg + iconPositionLeft) を自動でセットする HOC を
 * assets/js/button-line-defaults.js で実装。本ファイルではエディタ側にだけ enqueue する。
 * 属性名は Ollie Pro 互換で、描画は pvip-blocks の button-icons 拡張が行う
 * （Ollie Pro 有効時はそちらへ委譲される）。
 *
 * - エディタで属性を attribute に焼くため、フロントでも同じ icon を描画
